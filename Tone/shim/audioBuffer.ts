function installCopyFromChannel() {
    ...
    AudioBuffer.prototype.copyFromChannel = function(destination, channelNumber, startInChannel) {
        let source = this.getChannelData(channelNumber|0).subarray((startInChannel as any)|0);
...
function installCopyToChannel() {
    ...
    AudioBuffer.prototype.copyToChannel = function(source, channelNumber, startInChannel) {
        let clipped = source.subarray(0, Math.min(source.length, this.length - ((startInChannel as any)|0)));
        this.getChannelData(channelNumber|0).set(clipped, (startInChannel as any)|0);
    };
}

function audioBufferShim() {
